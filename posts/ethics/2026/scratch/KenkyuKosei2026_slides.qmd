...
# 1. 研究過程での新要素: AI(問題の概観){background-color="#010638"}
...
# 2. AIへの問い:「AIと研究公正」 {background-color="#010638"}
...
# 3. AI出力の制御 {background-color="#010638"}

----

## 生成前に防ぐ(誤りを作らせない){background-color="#010638"}

[Hallucination対策スライド](Hallucination2026_slides.html)の手法整理 — 各手法に例と効果

::: {style="font-size:60%;"}
1. **外部ツール実行** — R/Pythonを書かせて*実行*し、生成でなく計算結果から数値を得る
    * 例: [ellmer](https://ellmer.tidyverse.org/), [ChatGPT Code Interpreter](https://platform.openai.com/docs/assistants/tools/code-interpreter), [Claude Code](https://claude.ai/claude-code)
    * 効果: 計算・集計系の誤りを根絶 / ✗文章・理論の解釈には無力
2. **RAG** — 自分の文献・データを検索し、ヒット文を注入してから回答
    * 例: [NotebookLM](https://notebooklm.google.com/), [Elicit](https://elicit.com/), [Scite](https://scite.ai/)
    * 効果: 事実誤認を大幅削減・出典提示しやすい / ✗検索源の偏り・取り違えに従属
3. **出典強制** — 「提供文のみに基づき各主張に引用文を対応づけよ」と指示
    * 例: [Consensus](https://consensus.app/), [Claude Citations API](https://docs.anthropic.com/en/docs/build-with-claude/citations)
    * 効果: 検証可能性が高い / ✗出典の*実在*は別途確認要
4. **モデル横断照会** — 系統の異なる複数モデルに同じ問いを投げ、一致/不一致を比較
    * 例: [Poe](https://poe.com/), [OpenRouter](https://openrouter.ai/docs/overview), Claude+Gemini+ChatGPT手動比較
    * 効果: 単一モデルの系統的誤りを検出 / ✗全モデル共通の誤りには無力
:::

----

## 生成後に捕まえる(誤りを発見する){background-color="#010638"}

::: {style="font-size:60%;"}
5. **Self-Verification (CoVe)** — 検証質問を生成→個別に答え直し→矛盾を修正
    * 例: [SelfCheckGPT](https://github.com/potsawee/selfcheckgpt), [Sant'Anna /verify-claims](https://github.com/pedrohcgs/claude-code-my-workflow)
    * 効果: 単純な論理・計算ミスを修正 / ✗知識不足・自信満々の誤りは見逃す
6. **不確実性開示・棄権** — 確信度を出力させ、閾値未満は「分からない」と言わせる
    * 例: [Semantic Entropy (Nature 2024)](https://www.nature.com/articles/s41586-024-07421-0), [OpenAI logprobs](https://cookbook.openai.com/examples/using_logprobs)
    * 効果: 高リスク用途で誤答率低下 / ✗有用性低下・棄権が報酬されにくい
7. **出典実在確認** — 提示されたDOI/URLを*人間が*開き、実在と内容を照合
    * 例: [rcrossref](https://docs.ropensci.org/rcrossref/), [scite](https://scite.ai/), [DOI.org](https://doi.org/)
    * 効果: 捏造引用を確実に排除 / ✗手間・automation bias
:::


----

## 例: モデル横断照会(手法4){background-color="#010638"}

* 同じ問い「AIと研究公正」をClaude / Gemini / ChatGPTに投げた(&rarr; 前節)
* <Blue>効果</Blue>: 系統差で強調点が分散
    * Claude=検証・記録、Gemini=データ植民地主義、ChatGPT=透明性
    * 3モデルの<Red>共通項</Red>(検証・透明性)が信頼できる核
* <Orange>限界</Orange>: 全モデルが同じ偏りを共有すれば無力

----

## 例: 批判を強制する(adversarial prompting){background-color="#010638"}
...
* <Blue>効果</Blue>: sycophancyを看破させ、隠れた前提・反証を引き出せる

---

## 例: RAG(手法2) — 情報源を固定しても偏る{background-color="#010638"}
...
## 例: RAG — 似たプロンプトで正反対の要約{background-color="#010638"}
...
<Orange>効果の限界</Orange>: source-groundedでも依拠論文の選択で結論が反転 &rarr; 検索源の偏りに従属

---

## 例: RAG — 研究者による補正{background-color="#010638"}
...
* <Blue>効果</Blue>: 母集団の網羅・重み付けを明示させ、RAGの偏りを是正

---

## 例: Self-Verification CoVe(手法5){background-color="#010638"}
...
* <Blue>効果</Blue>: 自信満々の捏造主張を、一次資料との照合で除去

----

## 商用例: [Refine.ink](https://www.refine.ink/)(手法5の産業化){background-color="#010638"}

経済学者(Ben Golub他)が作った査読前レビュー。1論文を約20-40分で「referee」のように読む

::::{.incremental}
* <Blue>得意</Blue>: 内部論理の照合 — 証明の穴、記号の不整合、表間の矛盾、相互参照
    * LaTeX入力、理論系(econ theory・数学・形式哲学)に最適
* <Blue>評価</Blue>: Fudenberg(MIT)・Lederman・Tamuz(Caltech)が推薦、印刷物で<Red>87本</Red>が謝辞
    * Cochrane「キャリアで受けた最良級のコメント」/ ただし独立したRCT評価は無し(=推薦ベース)
* <Red>しない</Red>: 出典検証・図の解析・雑誌適合 → <Orange>捏造引用は素通り(手法7は人間)</Orange>
* <Red>境界</Red>: 理論向け。実証IO・労働の識別・表や途上国の制度的主張の<Red>真偽</Red>は検証しない
::::

::: {style="font-size:70%; margin-top:0.2em"}
含意: AIは検証の安い層(内部論理)を圧縮するが、高い層(出典・文脈)は人間が持つ。
:::

----

## まとめ: Verify, Disclose, Take Responsibility{background-color="#010638"}
...
## 2.と3.は簡単に実践可能{background-color="#010638"}
...
* 手動で専門知識を動員する
* 機微情報(調査・行政データ)はAI APIに入力しない(予防)


----

## 検証が要るならAIを使う意味は?{background-color="#010638"}

> 人間が検証するならAI使う必要あり?(&rarr; 前節の問い)

::::{.incremental}
* <Blue>非対称コスト</Blue>: AIが得なのは「検証 $<$ 生成」の作業のみ
    * 草稿・boilerplate R・候補50論文の抽出 → 検証は安い
    * 新規の識別戦略の証明 → 検証 $\approx$ 生成、利得は小さい
* <Blue>比較優位</Blue>: AI=広い再現率(recall)、人間=判断・精度(precision)
    * RAのregressionも確認する。確認する=無価値ではない(AI $<$ RA, $\neq 0$)
* <Blue>検証は選択的</Blue>: 全再現でなく、論証を支える事実主張のみ照合
* <Red>境界(本研究の核心)</Red>: 「検証する専門性」が無いとこの議論は崩れる
    * 途上国の制度・文脈は学習データで過少代表 → 研究者が安く検証できない
    * AIが最も不正確で、利用者が最も検証しにくい領域が重なる
::::

::: {style="font-size:70%; margin-top:0.3em"}
AIは「検証が生成より安い」かつ「検証できる専門性がある」ときに有用。途上国研究は後者が崩れやすい。
:::
