...
format:
  revealjs: 
    ...
    preview-links: auto
    logo: RI.png
    footer: <http://seiroito.github.io/test/posts/ethics/2026/KenkyuKosei2026_slides.html>
    theme: sky
####    css: tinytable.css
execute:
...
resources:
  - KenkyuKosei2026_slides.pdf
#### quarto render KenkyuKosei2026_slides.qmd
---
...
::: {.center2}
[http://seiroito.github.io/test/posts/ethics/2026/KenkyuKosei2026_slides.html](http://seiroito.github.io/test/posts/ethics/2026/KenkyuKosei2026_slides.html)
:::
...
# 社会が研究する目的=既存の知識を修正しながら新知識を蓄積すること。{background-color="#010638"}

# 研究公正のルール=目的を実現する手段。{background-color="#010638"}

<!--
# 学術的根拠をもとに研究し、作業内容と得た成果を歪めずに発表すること。{background-color="#010638"}
-->

# 「誰が」「何をやって」「何が分かった」=知識、を検証可能な状態で蓄積。{background-color="#010638"}


----

## なぜ知識を検証可能な状態にしておくか{background-color="#010638"}

:::: {.fragment}
* 分野全体でやり直し: 再現性危機replication crisis(2010年代以降)
    * 心理学: 2008年掲載のトップ3誌100論文で36%[@OSC2015]
    * 癌研究: 2010-2012掲載のトップ53論文で`r round((23/53)*100, 0)`%
* 心理学では1960年代から指摘: 小標本(検定力statistical powerの低い)研究が50年以上続く
::::

:::: {.fragment}
行動経済学に再現性危機がなかった理由

* 実験経済学の伝統を受け継いだため [[video](https://youtu.be/YUtNAYvxfRI?si=BAcHgyWPl-guCr8f&t=2881)]
    * 論文=文章+実験指示書+データ+コード
    * 掲載条件: 過去の実験結果を研究対象で再現+新規実験
* 研究(作業)の透明性 &rArr; 知識を修正しながら積み上げ
* 行動経済学に限られない
::::
...
::::: {.column width = "60%"}
::: {style="font-size:130%"}
<DarkGray> <LightBlue>F</LightBlue><Orange>abrication ねつ造</Orange>「存在しないデータや研究成果を作成すること。」</DarkGray>

<DarkGray><LightBlue>F</LightBlue><Orange>alsification 改ざん</Orange>「操作」によって「データ」や「成果などを真正でないものに加工すること」。</DarkGray>

<DarkGray><LightBlue>P</LightBlue><Orange>lagerism 剽窃、盗用</Orange> 他の研究者の「アイディア」「方法」「データ」「結果」を「了解または適切な表示なく流用すること」。</DarkGray>
:::
:::::
...
:::::::::: {.columns}
 「誰が」「何をやって」「何が分かった」を検証可能な状態で蓄積する
 
::::: {.column width = "30%"}
::: {.incremental}
::: {style="font-size:130%"}
* ギフト・オーサーシップ ([CRediT](https://www.elsevier.com/authors/policies-and-guidelines/credit-author-statement))([Vancouver](https://www.icmje.org/recommendations/browse/roles-and-responsibilities/defining-the-role-of-authors-and-contributors.html#two))  
* 利益相反によるバイアス  
* データの未保存や再現性の欠如  
* 不適切な統計手法  
* 論文の多重投稿  
...
<!--
* 再現性がない &rArr; 成果が疑問視 &rArr; 発見が知識にならない
...
:::::::::: {.columns}
「誰が」<DarkGray>「何をやって」「何が分かったか」を検証可能な状態で蓄積する</DarkGray>
...
::::: {.column width = "68%"}
::: {.incremental}
* GAは贈収(賄)双方に便益あり、なくなりにくい
* でも、[共著者リスク](https://pmc.ncbi.nlm.nih.gov/articles/PMC2541639/)も[付いてくる](https://www.icmje.org/news-and-editorials/new_rec_aug2013.html#:~:text=The%20addition%20of%20a%20fourth%20criterion%20was,part%20of%20the%20paper%3B%20ask%20someone%20else%E2%80%9D)&larr; Asad IslamのFF疑惑
* パワハラ・圧力による盗用なら(FF)P...利益配分&larr;研究倫理
:::
:::::
::::::::::
...
::: {.incremental}
* 研究公正を損ねる条件下の共著者はいるか
    * 特定方向の成果に利益を見出す関係者と縁が深い
       * 雇用者・METI、資金・便宜提供者、研究者(自身も)
    * 顕著な成果、強い(矛盾のない)成果を求めている
* 大物追従志向はないか
    * 例: 早期児童発達介入は非認知能力を高める、プライミング、最貧困層貸付にはトレーニングが必須
    * 反証すると大物がエディタの雑誌には投稿できない
:::
:::::
::::::::::
...
:::::::::: {.columns}
<DarkGray>「誰が」</DarkGray><span style="color:white;">「何をやって」「何が分かったか」を検証可能な状態で蓄積する</span>
...
::::: {.column width = "68%"}
::: {.incremental}
* 再現性がない &rArr; 発見が知識にならない
    * 追試検証ができない
    * 成果が疑問視
* 論文掲載が困難: 多くの雑誌で再現パッケージも投稿必須
:::
:::::
::::::::::
...
:::::::::: {.columns}
<DarkGray>「誰が」「何をやって」</DarkGray>「何が分かったか」を検証可能な状態で蓄積する

::::: {.column width = "30%"}
::: {.incremental}
::: {style="font-size:130%"}
* <DarkGray>ギフト・オーサーシップ ([CRediT](https://www.elsevier.com/authors/policies-and-guidelines/credit-author-statement))([Vancouver](https://www.icmje.org/recommendations/browse/roles-and-responsibilities/defining-the-role-of-authors-and-contributors.html#two))  </DarkGray>
* <DarkGray>利益相反によるバイアス  </DarkGray>
* <DarkGray>データの未保存や再現性の欠如  </DarkGray>
* <DarkGray>不適切な統計手法  </DarkGray>
:::
:::
:::::
...
<Orange>例5:</Orange> ミクロ開発経済学研究者[Asad Islam](https://research.monash.edu/en/persons/asadul-islam/)の複数論文で[データ不正(FF)の疑い](https://a-ortmann.medium.com/the-asad-islam-gdri-saga-f96b567c93d6)  
...
* Journal of Risks and Uncertainty
...
::::: {.column width = "30%"}
:::::: {style="font-size: 70%;"}
* List on left may be imprecise. (Hard to follow...descriptions are scattered across sites.)
* [大学の調査結果](https://www.theaustralian.com.au/subscribe/news/1/?sourceCode=TAWEB_WRE170_a_DDG&dest=https%3A%2F%2Fwww.theaustralian.com.au%2Fhigher-education%2Fmonash-professor-asad-islam-cleared-of-data-fabrication-allegations-after-investigation%2Fnews-story%2F4017ef7bff53ad9364dd33a5dc1fb340)
    * [研究不正(fabrication)なし](https://a-ortmann.medium.com/the-asad-islam-gdri-saga-f96b567c93d6#:~:text=it%20could%20not%20find%20evidence%20of%20data%20fabrication.)
    * QRPあり
* 大学での処分なし
* Fundingは途中終了curtailed
* Field operatorなので、予算なしに研究は困難
::::::
::::: 
...
:::: {.incremental}
 * 成果を冗長に書く 
 * 要約が不正確 ([DNSRI](https://htmlpreview.github.io/?https://github.com/SeiroIto/PaperNotes/blob/main/QRPsInDutchNationalSurvey_Tufte.html)07) 
 * どのような過程を踏んだか明記しない ([DNSRI](https://htmlpreview.github.io/?https://github.com/SeiroIto/PaperNotes/blob/main/QRPsInDutchNationalSurvey_Tufte.html)10) 
 * 図表の脚注に留意事項を説明するが本文は無記載 ([DNSRI](https://htmlpreview.github.io/?https://github.com/SeiroIto/PaperNotes/blob/main/QRPsInDutchNationalSurvey_Tufte.html)11) 
 * 研究なのにダークパタン  
 * 過去著作の数段落を明示せずコピー、ただし、参考文献には掲載 ([DNSRI](https://htmlpreview.github.io/?https://github.com/SeiroIto/PaperNotes/blob/main/QRPsInDutchNationalSurvey_Tufte.html)02)  
::::
::::: 
:::::::::: 
...
::::: {.column width = "50%"}
:::: {.incremental}
* デザイン不十分(によるstatiscal power不足)を認め、<Red>short paperやウェブ論考に</Red>
* <Red>共著者を厳選する</Red>
::::
::::: 
:::::::::: 
...
アジ研研究者にとって可能性が高そうなQRP

::::: {.incremental}
* 共著者が怪しいことをしている(かも)
    * QRPの責めはcorresponding authorおよび[全共著者に向かう](https://www.icmje.org/news-and-editorials/new_rec_aug2013.html#:~:text=The%20addition%20of%20a%20fourth%20criterion%20was,part%20of%20the%20paper%3B%20ask%20someone%20else%E2%80%9D)
      ...
      — <a href="https://www.icmje.org/news-and-editorials/new_rec_aug2013.html">The New ICMJE Recommendations (August 2013)</a>
      </div>
      :::
      ::::::
      
    * <Red>研究者が共著者を厳選</Red>すべき
:::::
::::
...
::::: {.incremental}
* 研究関係者から研究公正を貫くことを妨害される
    * 例: 執筆者順などでパワハラ・強弁・忖度を受ける・する
    * 例: 研究対象や監督官庁(に忖度した誰か)から論旨変更を迫られる
    * <Red>研究者が再現可能性に留意</Red>すべき
    * <Red>研究者は自らの安全を最優先</Red>すべき
    * <Red>雇用者が真実発信ができる研究環境を保障</Red>すべき
:::::
...
::::: {.incremental}
* 時間がないのでAIに論文執筆の枢要部分を委ねる
    * 研究公正: 著者は研究のすべてに責任を負う&rarr;AIに枢要部分を任せて負えるか?
    * <Red>研究者が枢要部分を指示</Red>すべき
    * <Red>Hallucination, overconfidence, lack of attributionなど原稿を人間がチェック</Red>すべき
    * <Red>かつ、AI利用を明記し、入力と出力を開示</Red>すべき
    * <Red>そして、作業や解釈に関する詳細な質問に対して準備</Red>すべき
:::::
...
* 制御の技量が試される
  * RA: 指導、共著者として責任を共有
  * AI: [`feedback rules`](https://code.claude.com/docs/en/memory#organize-rules-with-claude/rules/)と[`skills`](https://claude.com/docs/skills/overview)で誘導、人間が検証(一次資料+専門知識)し全責任を持つ
::::
...
* 最終成果に近い作業まで使う人もいる
  ...
  * 下書き[(自分ぽく)](https://claudeblattman.com/essentials/voice/#teaching-ai-your-voice)
...
* 「検証費用<作成費用ならば使うべき」とAIは回答(下記も回答)
  * 一次資料の検証
  * 専門(現地)知識による検証&rarr;<Red>難易度高い</Red>: AIの学習コーパスに入りにくく、自分の専門分野以外だと検証が難しいから
::::::
::::
...
:::: {.fragment}
違うモデルやエージェントにcross checkさせる&rarr;<Red>最後は人間がチェック</Red>
::::
...
## AI利用の心構え{background-color="#010638"}
...
:::::::::: {.columns}
::::: {.column width = "50%"}
[Deck 1](Deck1_TheGraduationModelBlueprint.pdf)![](NotebookLM_SlideDeck1.jpg){height=80}
...
::::: {.column width = "50%"}
[Deck 2](Deck2_MicrocreditAndPovertyRealities.pdf)![](NotebookLM_SlideDeck2.jpg){height=80}
...
* AIがアクセスできるものとそれ以外をpermissions (in AI or at OS filesystem), rulesで明確に分ける
    * フォルダ
    * 変数名
...
* 厳罰のあるFFPは割に合わないが、罰則が弱いQRPは自分が気をつけるしかない
* 「時間がない」なら、早めに上長に相談を
* 共著者は正直者を選ぶ
* 研究者は研究公正よりも(自らの)安全を最優先すべき
