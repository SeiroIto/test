...
$$
\begin{aligned}
\textrm{XPT}_{j}\uparrow &\Rightarrow W_{j}\uparrow && (Table 2, Figure 5)\\
\Delta\textrm{XPT}_{j}\uparrow &\Rightarrow \Delta W_{j}\uparrow && (Table 4A, Figure 10)\\
\Delta\textrm{Task}\uparrow &\Rightarrow \Delta W\uparrow && \textrm{(Table 4A, Figure 10)}\\
\Delta\textrm{Task}\uparrow &\Rightarrow \Delta \textrm{Emp}\uparrow && \textrm{(Table 5, Figure 12)}
\end{aligned}
$$

    <Per80>$\Delta\textrm{Task}$</Per80>&uarr; &rArr; $\Delta$Emp&uarr; $p \approx 0.74$ (Table 5C).

* Supply effects
   *   <Per80>$\Delta\textrm{XPT}$</Per80>&uarr; &rArr; $\Delta$Emp&darr; ($\because$ number of qualified workers&sarr;, Table 5, Figure 12)
* Inexpert routine task automation (Table 8, Figures 14 & 15)
   *   <Per80>$\Delta\widetilde{\textrm{XPT}}$</Per80>&uarr; &rArr; $\Delta$W&uarr; ($\because$ the job becomes more focused on expert tasks).
   *   <Per80>$\Delta\widetilde{\textrm{XPT}}$</Per80>&uarr; &rArr; $\Delta$Emp&darr; ($\because$ barriers to entry rise). **Specific Reservation:** This employment effect is only statistically significant after trimming 4 outlier occupations, making the result sensitive to the sample.
* Expert routine task automation
   *   <Per80>$\Delta\widetilde{\textrm{XPT}}$</Per80>&darr; &rArr; $\Delta$W&darr; ($\because$ the core expertise requirement is removed).
   *   <Per80>$\Delta\widetilde{\textrm{XPT}}$</Per80>&darr; &rArr; $\Delta$Emp&uarr; ($\because$ barriers to entry fall). This predicted employment effect is not statistically significant, providing no evidence to support this specific causal pathway.

Consistent with the Expertise model (expertise changes act like supply shifts) and the Task model (task quantity changes act like demand shifts)

   * Evidence for some of the models' employment predictions is weak

<!--
We showed
$$
\eta^{\frac{\lambda-1}{\lambda}} \left( \frac{\bar{L}}{\bar{K}} \frac{\bar{I}}{1-\bar{I}} \right)^{-\frac{1}{\lambda}} e^{ \frac{\lambda-\bar{I}(\lambda-1)}{\lambda(1-\bar{I})(\lambda-1)} } 
>
r \left( \frac{1-\bar{I}}{\bar{L}/Y} \right)^{-\frac{1}{\lambda}} e^{ \frac{\lambda-\bar{I}(\lambda-1)}{\lambda(1-\bar{I})(\lambda-1)} } 
$$
so
$$
\eta > r \left( \frac{1-\bar{I}}{\bar{L}/Y} \right)^{-\frac{1}{\lambda}} e^{ \frac{\lambda-\bar{I}(\lambda-1)}{\lambda(1-\bar{I})(\lambda-1)} } 
$$

$$
\begin{aligned}
r &\left( \frac{1-I}{\bar{L}/Y} \right)^{-\frac{1}{\lambda}} e^{\frac{\lambda - I(\lambda-1)}{\lambda(1-I)(\lambda-1)}}\\
&\le Y \frac{I}{\bar{K}} \left( \frac{1-I}{\bar{L}/Y} \right)^{\frac{1}{\lambda}} e^{\frac{\lambda - I(\lambda-1)}{\lambda(1-I)(\lambda-1)}}\\
&< \left( \frac{\eta\bar{K}}{I} \right) \frac{I}{\bar{K}} \left( \frac{1-I}{\bar{L}/\left(\frac{\eta\bar{K}}{I}\right)} \right)^{\frac{1}{\lambda}}e^{\frac{\lambda - I(\lambda-1)}{\lambda(1-I)(\lambda-1)}}\\
&= \eta^{\frac{\lambda-1}{\lambda}} \left( \frac{\bar{L}}{\bar{K}} \frac{I}{1-I} \right)^{-\frac{1}{\lambda}} \exp\left\{ \frac{\lambda-I(\lambda-1)}{\lambda(1-I)(\lambda-1)} \right\} \quad \text{(B.6)}
\end{aligned}
$$
| Variable Definition                                                                                              | Math Expression                                                                                                                    |
| -----------------------------------------------------------------------------------------------------------------: | :--------------------------------------------------------------------------------------------------------------------------------- |
|                                                                                Occupation $j$'s mean expertise level | <span style="font-size: 80%;">$\textrm{XPT}_{j}$</span>                                                                            |
|                                                                               Occupation $j$'s total number of tasks | <span style="font-size: 80%;">$\textrm{Task}_{j}$</span>                                                                           |
...
| Task exposure (to automation): Fraction of tasks lost if all routine tasks are removed | <span style="font-size: 80%;">$\Delta\widetilde{\textrm{Task}}_{j} \equiv 1-\frac{N_{j,nr, 1977}}{N_{j, 1977}}$</span> |
